 ...
 */
$ajax_edit_question_url = plugins_url() . '/mfs-survey/ajax/ajax-edit-question.php';
...
<div class="wrap">
	<h2><?php _e('Edit Page', 'mfs-survey'); ?></h2>
	
	<form action="edit.php?page=mfs-survey/survey-action.php&amp;noheader=true" method="post" name="form_edit_page" id="form_edit_page" onsubmit="return validate_edit_page();" autocomplete="off">
		<table class="widefat page fixed" cellspacing="0">
			<thead>
				<tr>
					...
					<th>
						<?php _e('Current Page', 'mfs-survey'); ?>
					</th>
				</tr>
			</thead>
			<tbody>
		...
		<table class="widefat page fixed" cellspacing="0">
			<thead>
				<tr>
					<th><?php _e('Question', 'mfs-survey'); ?></th>
					<th><?php _e('Active', 'mfs-survey'); ?></th>
					<th width="5%"></th>
					<th width="5%"></th>
				</tr>
			</thead>
					...
					foreach( $results as $result ) {
						...
						echo "
							<td style='text-align:right;'>								
								<a href='admin.php?page=forms/edit-question-form.php&page_id=".$page_id."&question_id=".$question_id."'>".__('Edit', 'mfs-survey')."</a>
							</td>";
						
						// Delete link
						echo "
							<td style='text-align:right;'>
								<a href='javascript:void(0);' onclick='delete_question(\"$ajax_edit_question_url\", ".$page_id.", ".$question_id.", \"$current_page_url\");'>".__('Delete', 'mfs-survey')."</a>
							</td></tr>";
		...
		<table class="widefat page" cellspacing="0" id="tbl_question_<?php echo $question_id; ?>">
			<thead>
				<tr>
					<th colspan="3">
						<?php _e('Edit Next Page for Options', 'mfs-survey'); ?>
					</th>
				</tr>
			</thead>
			<tbody>
							...
							foreach ($value as $key => $value) {
								...
								echo '<td width="50%">
										<select id="sel_next_page'.$option_id.'" name="sel_next_page[]">
											<option value="">-- '.__('Please Choose', 'mfs-survey').' --</option>';
		...
		<p class="submit">
			<input type="submit" name="btn_edit_page" id="btn_edit_page" value="<?php _e('Save', 'mfs-survey'); ?>" class="button" />
		</p>
